...
// Usage: php persistGamesForYear.php username year
if ($argc < 3) {
    fwrite(STDERR, "Usage: php persistGamesForYear.php <username> <year>\n");
    exit(1);
}
...
$fromDate = "$year-01-01";
$toDate = "$year-12-31";
$authorizationToken = '459028ff-70d9-4e98-8fa1-6c8e07865eea';
...
$outputFile = "aggregated_plays_{$username}.json";
$existingGames = [];
...
// Merge: sum plays for same id, add new games
foreach ($games as $id => $game) {
    if (isset($existingGames[$id])) {
        $existingGames[$id]['plays'] += $game['plays'];
    } else {
        $existingGames[$id] = $game;
    }
}
// Save merged aggregation
file_put_contents($outputFile, json_encode(array_values($existingGames), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
echo "Aggregated plays saved to $outputFile\n";
